...
import {
  useUpdateSelectionNode,
  useDeleteSelectionNode,
...
// Roadmap 노드 카드(`RoadmapNodeCard`)와 동일 구조 · axisId + nodeId 로 편집/삭제.
...
export function SelectionNodeCard({ axisId, node, dragHandle }: Props) {
  const [mode, setMode] = useState<'view' | 'edit'>('view');
  const [title, setTitle] = useState(node.title);
  const [rationale, setRationale] = useState(node.rationale ?? '');
  const [body, setBody] = useState(node.body);
  const [inline, setInline] = useState<string | null>(null);
  const update = useUpdateSelectionNode(axisId, node.id);
  const del = useDeleteSelectionNode(axisId);
  ...
  const handleError = (err: unknown) => {
    if (err instanceof ApiError) {
      if (err.code === 'SELECTION_NODE_TITLE_BLANK') {
        setInline('노드 제목을 입력해주세요.');
      } else if (err.code === 'SELECTION_NODE_BODY_BLANK') {
        setInline('노드 본문을 입력해주세요.');
      } else if (err.code === 'SELECTION_NODE_NOT_FOUND') {
  ...
  const onSave = () => {
    ...
    update.mutate(
      ...
      { onSuccess: () => setMode('view'), onError: handleError },
    );
  };
      ...
      <div className="flex flex-1 flex-col gap-2">
        <header className="flex items-baseline justify-between gap-3">
          <div className="flex flex-col gap-0.5">
            ...
            <h4 className="text-sm font-semibold text-cream">{node.title}</h4>
          </div>
        ...
        {node.rationale && <p className="text-xs text-cream-mute">{node.rationale}</p>}
        <pre
